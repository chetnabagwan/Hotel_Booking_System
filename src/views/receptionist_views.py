...
from models.schemas import CheckinSchema,CheckoutSchema,ReceptionistSchema,ChangeDefaultPasswordSchema
...
def change_default_pswd(data:ChangeDefaultPasswordSchema,user:user_dependency):
    try:
        if user[0]!= Config.RECEPTIONIST:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail=Config.UNAUTHORIZED_USER)
        Receptionist.change_default_password(user[1],data.old_pswd,data.new_pswd)
    except Error:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,details="Incorrect old password")